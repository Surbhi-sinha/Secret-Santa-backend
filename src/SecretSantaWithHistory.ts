...
export class SecretSantaWithHistory implements ISecretSanta {
  ...
  assignSecretChild(employees: any[]): any[] {
    ...
    while (maxAttempts-- > 0) {
      let isValid = employees.every((emp, index) => {
        ...
        return (
          emp.Employee_Name !== newChild.Employee_Name && // No self-assignment
          previousAssignments.get(emp.Employee_Name) !== newChild.Employee_Name // No repeat from last time
        );
      });

      if (isValid) break; // Stop if all assignments are valid
